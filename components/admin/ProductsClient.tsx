...
export default function ProductsClient({ initialProducts, categories }: { initialProducts: Product[], categories: any[] }) {
  ...
  const openAdd = () => {
    setEditTarget(null)
    setForm({ name: '', description: '', price: '', discount_price: '', stock_quantity: '', sku: '', category_id: categories.length > 0 ? categories[0].id : '', is_deal: false })
    setShowForm(true)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-brand-gray font-[Inter,sans-serif] text-sm">{initialProducts.length} total products</p>
        </div>
        <button
          onClick={openAdd}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-fade-in">
            ...
            <div className="p-6 space-y-4">
              {/* Image Upload */}
              <div>
                <label className="block text-xs font-[Outfit,sans-serif] font-semibold text-brand-charcoal mb-1.5 uppercase tracking-wide">Product Image (Optional)</label>
                <div className="flex items-center gap-4">
                  {(form as any).imagePreview ? (
                    <img src={(form as any).imagePreview} alt="Preview" className="w-16 h-16 object-cover rounded-xl border border-brand-light-gray" />
                  ) : (
                    <div className="w-16 h-16 bg-brand-offwhite rounded-xl border border-dashed border-brand-light-gray flex items-center justify-center text-brand-gray">
                      <Package size={24} />
                    </div>
                  )}
                  <input
                    ...
                    className="text-sm text-brand-gray file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-brand-emerald/10 file:text-brand-emerald hover:file:bg-brand-emerald/20 transition-all outline-none"
                  />
                </div>
              </div>
            ...
            <div className="flex gap-3 px-6 pb-6">
              ...
              <button onClick={handleSave} disabled={isLoading} id="admin-save-product" className="flex-1 btn-primary py-2.5 flex items-center justify-center gap-2 disabled:opacity-50">
                <Save size={16} />
                {isLoading ? 'Saving...' : (editTarget ? 'Save Changes' : 'Add Product')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fade-in text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} disabled={isLoading} className="flex-1 btn-outline py-2.5">Cancel</button>
              <button
                onClick={() => handleDelete(deleteTarget)}
                disabled={isLoading}
                id="admin-confirm-delete"
                className="flex-1 bg-red-500 text-white font-[Outfit,sans-serif] font-semibold py-2.5 rounded-xl hover:bg-red-600 transition-colors disabled:opacity-50"
              >
